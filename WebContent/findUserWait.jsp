  ...
  <body>
    <form action="admin_findUserWait" method="post">
     ...
     <center>
       <table border="1">
          ...
          <c:forEach var="i" items="${requestScope.list}">
            <tr>
              ...
              <td>${i.username}</td>
              <td>${i.userphone}</td>
              <td>${i.jobapplication}</td>
              <td>${i.applicationstatus}</td>
            </tr>
          </c:forEach>
       </table>
       第${page.pageno}/${page.totalpage }页
       <a href="admin_findUserWait?page.pageno=1">首页</a>&nbsp;
       <c:choose>
        <c:when test="${page.pageno>1}">
       <a href="admin_findUserWait?page.pageno=${page.pageno-1 }">上一页</a>&nbsp;
        </c:when>
        <c:otherwise>
       ...
       <a href="admin_findUserWait?page.pageno=${page.pageno+1 }">下一页</a>&nbsp;
        </c:when>
        <c:otherwise>
       ...
       <a href="admin_findUserWait?page.pageno=${page.totalpage}">末页</a>&nbsp;
       总${page.totalcount}条
     </center>
  </body>
</html>